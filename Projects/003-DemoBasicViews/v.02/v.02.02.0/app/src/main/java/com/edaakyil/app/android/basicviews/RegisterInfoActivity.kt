...
class RegisterInfoActivity : AppCompatActivity() {
    ...
    private fun saveData(close: Boolean) {
        BufferedWriter(OutputStreamWriter(FileOutputStream(File(filesDir, "$USERS/${mRegisterInfo.username}.txt")), StandardCharsets.UTF_8)).use(::writeRegisterInfo)
    ...
    private fun selectOptionIfUserSaved(close: Boolean) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton(R.string.alert_dialog_update) { _, _ -> saveData(close); mButtonContinue.isEnabled = true }
    ...
    fun onLoadButtonClicked(view: View) {
        try {
            ...
            BufferedReader(InputStreamReader(FileInputStream(File(filesDir, "$USERS/$username.txt")), StandardCharsets.UTF_8)).use(::loadRegisterInfo)
