...
class RegisterInfoActivity : AppCompatActivity() {
    ...
    private fun saveData(close: Boolean) {
        BufferedWriter(OutputStreamWriter(FileOutputStream(File(filesDir, USERS_FORMAT.format("${mRegisterInfo.username}.txt"))), StandardCharsets.UTF_8)).use(::writeRegisterInfo)
    ...
    fun onLoadButtonClicked(view: View) {
        try {
            ...
            BufferedReader(InputStreamReader(FileInputStream(File(filesDir, USERS_FORMAT.format("$username.txt"))), StandardCharsets.UTF_8)).use(::loadRegisterInfo)
